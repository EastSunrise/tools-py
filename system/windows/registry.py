...
def register_protocol(protocol: str, delegate: str):
    if not protocol or protocol.strip() == '':
        log.warning('protocol is empty')
        return
    protocol = protocol.strip()
    if not re.match(r'^[a-z]+$', protocol):
        log.warning('protocol only accepts lowercase letters: [%s]', protocol)
        return
    delegate = os.path.abspath(delegate)
    if not os.path.exists(delegate):
        log.warning('delegate executable does not exist: [%s]', delegate)
        return
    ...
    log.info('protocol [%s] was registered with delegate executable [%s]', protocol, delegate)
...
def open_url(protocol: str, url: str):
    ...
    if not url.startswith(protocol + '://'):
        log.warning("the url doesn't match the protocol: [%s]", protocol)
    ...
    if delegate == '':
        log.warning('the delegate executable is not set')
        messagebox.showwarning('Warning', 'Cannot find an executable to open the url')
        return

    log.info('open url [%s] with [%s]', url, delegate)
